...
def verify_create_response(response, create_request, expected_origin):
    ...
    if origin != expected_origin:
        raise Exception(
            f"Origin {origin} does not match original request ({expected_origin}). Rejecting."
        )
    ...
    expected_rp_id_hash = hashlib.sha256(
        create_request["rp"]["id"].encode("utf-8")
    ).digest()

    if not hmac.compare_digest(auth_data.rp_id_hash, expected_rp_id_hash):
        raise Exception(
            "Relying party in authenticator data does not match request. Rejecting."
        )

    # Verify that the User Present bit of the flags in authData is set.
    if "UP" not in auth_data.flags:
        raise Exception(
            "User presence was not asserted by the authenticator. Rejecting."
        )

    if (
        create_request.get("authenticatorSelection", {}).get("userVerification")
        == "required"
        and "UV" not in auth_data.flags
    ):
        raise Exception(
            "User verification is required but was not asserted by the authenticator. Rejecting."
        )
    ...
    if alg not in (p["alg"] for p in create_request["pubKeyCredParams"]):
        raise Exception(
            "Public key algorithm not in list of accepted key types. Rejecting."
        )
    ...
    if alg == COSE_ALG_ECDSA:
        if kty != COSE_KTY_EC2:
            raise Exception(
                f"Invalid key type specified: expected {COSE_KTY_EC2} (EC2), received {kty}"
            )
    elif alg == COSE_ALG_EDDSA:
        if kty != COSE_KTY_OKP:
            raise Exception(
                f"Invalid key type specified: expected {COSE_KTY_OKP} (OKP), received {kty}"
            )
    ...
    if fmt == "none":
        ...
        if "x5c" in att_stmt:
            if att_alg == COSE_ALG_ECDSA:
                signing_cert = x509.load_der_x509_certificate(
                    att_stmt["x5c"][0].tobytes()
                )
                assert signing_cert.version == x509.Version.v3
                try:
                    fido_oid = signing_cert.extensions.get_extension_for_oid(
                    ...
                    pass
                assert (
                    signing_cert.extensions.get_extension_for_oid(
            ...
            if alg != att_alg:
                raise Exception(
                    "Self-attestation is in use, but credential algorithm and attestation algorithm do not match. Rejecting."
                )
            if alg == COSE_ALG_ECDSA:
        ...
        if len(x5c) != 1:
            raise Exception(
                f"Expected a single attestation certificate in fido-u2f attestation, received {len(x5c)}"
            )
        att_cert = x5c[0]
        ...
        if (
            not isinstance(signing_key, ec.EllipticCurvePublicKey)
            or not signing_key.curve.name == ec.SECP256R1.name
        ):
            raise Exception(
                "Signing key for FIDO U2F attestation is not a valid P-256 public key."
            )
...
def verify_get_response(credential, options, expected_origin, cred_lookup_fn):
    ...
    credential_record = cred_lookup_fn(user_handle, credential["rawId"])
    if not credential_record:
        msg = f"No credential for user found matching credential ID `{credential['id']}` and "
        if user_handle:
            msg += f"user handle `{user_handle}`"
        else:
            msg += "no user handle was given"
        raise Exception(msg)
    ...
    if allow_credentials:
        ...
        #     credential record.
        if credential["rawId"] != credential_record["cred_id"]:
            raise Exception("Credential ID does not match expected ID")
        #     If response.userHandle is present, verify that it equals the user handle of the user account.
        if user_handle and user_handle != credential_record["user_handle"]:
    ...
    # initiated,
        ...
        #     whose id equals credential.rawId. Let credentialRecord be that
        #     credential record.
        # identified_user = next(u for u in users if u['user_handle'] == user_handle and u['cred_id'] == credential['id'])
        if credential["rawId"] != credential_record["cred_id"]:
            raise Exception("Credential ID does not match expected ID")
        if user_handle != credential_record["user_handle"]:
            raise Exception("Unexpected user handle received from credential")
    else:
        raise Exception(
            "User is unidentified and no user handle was returned by credential"
        )
    ...
    if C["type"] != "webauthn.get":
        raise Exception(
            f"Invalid operation type asserted by credential: {C['type']}. Rejecting."
        )
    ...
    if C["origin"] != expected_origin:
        raise Exception(
            f"Attested origin `{C['origin']}` does not match expected origin `{expected_origin}`"
        )
    ...
    if not auth_data.has_flag("BE") and auth_data.has_flag("BS"):
        raise Exception(
            "Conflicted backup state: Authenticator reported to be backed up, but not backup-eligible. Rejecting."
        )
    ...
    #     set.
    if credential_record["backup_eligible"] and not current_be:
        raise Exception(
            "Authenticator previously reported that it was backup eligible on creation, but now does not. Rejecting."
        )
    #     If credentialRecord.backupEligible is not set, verify that currentBe
    #     is not set.
    elif not credential_record["backup_eligible"] and current_be:
        raise Exception(
            "Authenticator attempted to upgrade to be backup eligible. Rejecting."
        )
    ...
    # Compatibility.
    _cose_verify(
        credential_record["pub_key"], sig_bytes, auth_data_bytes + client_data_hash
    )
    ...
    user_sc = credential_record["sign_count"] if credential_record["sign_count"] else 0
    if sc > 0 or user_sc > 0:
        ...
        #             generated at the authenticator.
            ...
            # TODO: add policy
            raise Exception(
                "Authenticator signature count too low and the authenticator may have been cloned. Rejecting."
            )
...
def _cose_verify(cose_key: bytes, signature: bytes, data: bytes):
    ...
    if cose_alg == COSE_ALG_ECDSA:
        if kty != COSE_KTY_EC2:
            raise Exception(
                f"Invalid COSE key type specified for ECDSA: expected {COSE_KTY_EC2} (EC2), received {kty}"
            )
        ...
        if kty != COSE_KTY_OKP:
            raise Exception(
                f"Invalid COSE key type specified for EdDSA: expected {COSE_KTY_OKP} (OKP), received {kty}"
            )

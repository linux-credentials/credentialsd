...
use libwebauthn::{transport::{hid::HidDevice, Device}, webauthn::{WebAuthn, Error as WebAuthnError}, UxUpdate};
use tracing::debug;

use crate::{dbus::{CredentialRequest, CredentialResponse, GetAssertionResponseInternal, MakeCredentialResponseInternal}, tokio_runtime};

use super::AuthenticatorResponse;

pub(crate) trait UsbHandler {
    type Stream: Stream<Item = UsbState>;
...
impl Stream for LocalUsbStateStream {
    ...
    fn poll_next(self: std::pin::Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> std::task::Poll<Option<Self::Item>> {
        ...
        let prev_usb_state = self.usb_state;
                    ...
                    Some(UsbUvMessage::ReceivedCredential(response)) => {
                        match response {
                            AuthenticatorResponse::CredentialCreated(r) => {
                                ...
                                Ok(CredentialResponse::CreatePublicKeyCredentialResponse(
                                    MakeCredentialResponseInternal::new(
                                        r,
                                        vec![String::from("usb")],
                                        String::from("cross-platform"),
                            ...
                            AuthenticatorResponse::CredentialsAsserted(r) => {
                                ...
                                assert!(!r.assertions.is_empty());
                                if r.assertions.len() == 1 {
                                    Ok(CredentialResponse::GetPublicKeyCredentialResponse(
                                        GetAssertionResponseInternal::new(
                                                r.assertions[0].clone(),
                                                String::from("cross-platform"),
                                        )
        ...
        self.usb_state = next_usb_state.clone();
        Ok(next_usb_state)
    }
...
impl UsbUvHandlerTrait for UsbUvHandler {
    async fn notify_ceremony_completed(&self, response: AuthenticatorResponse) {
        self.signal_tx

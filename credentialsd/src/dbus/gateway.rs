...
use crate::{
    ...
    webauthn::{AppId, Origin, RequestEnvironment},
};
...
async fn validate_app_details(
...
) -> Result<(RequestingApplication, RequestEnvironment), Error> {
    ...
    let request_env = if let Some(claimed_origin) = claimed_origin {
        ...
        RequestEnvironment::SameOrigin(Origin::AppId(app_id))
    };
...
fn check_origin_from_app<'a>(
...
) -> Result<RequestEnvironment, WebAuthnError> {
    let trusted_clients = [
        "org.mozilla.firefox",
    ...
    if is_privileged_client {
        ...
        Ok(RequestEnvironment::SameOrigin(Origin::AppId(
            app_id.clone(),
        )))
    }
}
...
fn check_origin_from_privileged_client(
    ...
    match (origin, top_origin) {
        (origin @ Origin::Https { .. }, None) => Ok(RequestEnvironment::SameOrigin(origin)),
        (origin @ Origin::Https { .. }, Some(top_origin @ Origin::Https { .. })) => {
            if origin == top_origin {
                Ok(RequestEnvironment::SameOrigin(origin))
            } else {
                Ok(RequestEnvironment::CrossOrigin((origin, top_origin)))
            }
        }
        _ => {
...
mod test {
    ...
    use crate::webauthn::{Origin, RequestEnvironment};

    use super::check_origin_from_privileged_client;
    fn check_same_origin(origin: &str) -> Result<RequestEnvironment, WebAuthnError> {
    ...
    fn test_only_https_origins() {
        assert!(matches!(
            ...
            Ok(RequestEnvironment::SameOrigin(Origin::Https { host, .. })) if host == "example.com"
        ))
    }

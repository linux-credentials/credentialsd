...
#[derive(Debug)]
pub(crate) enum RequestEnvironment {
    SameOrigin(Origin),
...
impl RequestEnvironment {
    pub(crate) fn origin(&self) -> &Origin {
        match self {
            RequestEnvironment::SameOrigin(origin) => origin,
            RequestEnvironment::CrossOrigin((origin, _)) => origin,
        }
    }

    pub(crate) fn top_origin(&self) -> Option<&Origin> {
        match self {
            RequestEnvironment::SameOrigin(_) => None,
            RequestEnvironment::CrossOrigin((_, ref top_origin)) => Some(top_origin),
        }
    }
}
...
mod tests {
    ...
    use super::{format_client_data_json, Operation, RequestEnvironment};
    #[test]
    fn test_same_origin_client_data_json_str() {
        ...
        let json = format_client_data_json(
            Operation::Create,
            "abcd",
            &RequestEnvironment::SameOrigin("https://example.com".parse().unwrap()),
        );
    ...
    fn test_cross_origin_client_data_json_str() {
        ...
        let json = format_client_data_json(
            Operation::Create,
            "abcd",
            &RequestEnvironment::CrossOrigin((
